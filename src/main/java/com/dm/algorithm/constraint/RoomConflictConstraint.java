...
public class RoomConflictConstraint implements HardConstraint {
    ...
    public int check(Chromosome chromosome) {
        int conflicts = 0;
        Map<String, Map<Integer, List<Gene>>> roomSchedule = new HashMap<>();

        for (Gene gene : chromosome.getGenes()) {
            if (gene.getRoomId() == null)
                continue;

            roomSchedule
                    .computeIfAbsent(gene.getRoomId(), k -> new HashMap<>())
                    .computeIfAbsent(gene.getTimeslot(), k -> new ArrayList<>())
                    .add(gene);
        }

        for (Map<Integer, List<Gene>> slotMap : roomSchedule.values()) {
